...
MIDI device to a Windows MIDI Services loopback endpoint. Primary tested target
is a Roland FP-90X digital piano. Secondary target is any BLE MIDI peripheral
that follows the Apple 2015 BLE-MIDI 1.0 spec.

Flow:

    [ FP-90X piano ] <—BLE MIDI—> [ this bridge ] <—WinMM—> [ WMS loopback ]
                                                                     ^
                                                                     |
                                                      DAW / Chrome Web MIDI /
                                                          MIDI-OX, etc.
...
## Repo layout
    ...
    PerfectBluetoothMidi\
        ...
        CliHost.cs                 ← headless CLI for scripted debugging (--scan, --connect, --detect-channels, --phase, …)
        ...
        DeviceSettings.cs          ← per-MAC settings persistence in %AppData%\PerfectBluetoothMidi\devices.json
